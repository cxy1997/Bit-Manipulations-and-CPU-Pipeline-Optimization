...
#Description:
#Our task is to add a new instruction: iaddl
#The instruction has two operands: a register and a constant number.
#It will add the register value and the constant number and store the result to the register.
#Things to be modified in this file:
#1.add iaddl to valid code.
#2.state that iaddl needs both constant and register operand
#3.state which register to fetch in ID stage
#4.state the input of ALU in EX stage
#5.set conditon codes.
#Actually this task is similiar to modify seq-full.hcl.
...
# Add IIADDL here to make the instruction invalid.
...
# One of the operand of iaddl is register, so add IIADDL here 
bool need_regids =
	f_icode in { IRRMOVL, IOPL, IPUSHL, IIADDL,IPOPL,
		     IIRMOVL, IRMMOVL, IMRMOVL };

# Does fetched instruction require a constant word?
# One of the operand of iaddl is immediate number, so add IIADDL here 
bool need_valC =
	f_icode in { IIRMOVL, IRMMOVL, IMRMOVL, IJXX, IIADDL,ICALL };
...
# iaddl used the second operand as register ID, which is saved in D_rB
int d_srcB = [
...
#iaddl saved the result in same register it used
int d_dstE = [
...
# the first operand for iaddl is constant, which is saved in E_valC
int aluA = [
...
# the second operand for iaddl is register value, which is saved in E_valB
int aluB = [
...
# Condition codes will be setted for iaddl
