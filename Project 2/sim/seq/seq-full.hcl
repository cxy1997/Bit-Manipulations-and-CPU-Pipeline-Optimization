...
#3.state which register to fetch
#4.state the two input of ALU
...
# iaddl used the second operand as register ID, which is specified in rB field 
int srcB = [
...
# the first operand for iaddl is constant, which is saved in valC
int aluA = [
...
# the second operand for iaddl is register value, which is saved in valB
int aluB = [
